...
def to_float_es(col: pd.Series) -> pd.Series:
    if col.dtype != "object":
        return col
    # Quita separador de miles ".", cambia coma por punto y remueve "%"
    txt = (col.astype(str)
              .str.strip()
              .str.replace('.', '', regex=False)   # "1.234,56" -> "1234,56"
              .str.replace(',', '.', regex=False)  # "1234,56"  -> "1234.56"
              .str.replace('%', '', regex=False))  # "45%" -> "45"
    return pd.to_numeric(txt, errors='coerce')

class PlayerClustering:
    def __init__(self, dataset_path="df_delanteros.csv"):
    ...
    "Asistencias y creación de chances",
        ...
        self.dataset = self.dataset[self.columns].set_index(["Player", "Team"]).copy()
        self.dataset = self.dataset.apply(to_float_es)
    ...
    def preprocess_data(self):
        ...
        X_goles = pca_goles.fit_transform(X_scaled_goles)
    ...
    def get_suggested_players(self, jugador_objetivo):
        ...
        vector_objetivo = self.dataset.loc[jugador_objetivo].values.reshape(1, -1)

        # Paso 3: calcular distancias a todos los jugadores
        distancias = euclidean_distances(self.dataset.values, vector_objetivo).flatten()
        ...
        df_jugadores_similares = pd.merge(df_indices, self.dataset, on=["Player", "Team"], how="left")
        
        return df_jugadores_similares


# cluster = PlayerClustering()
# player = ("A. Martínez","Racing Club")
# similar_players = cluster.get_suggested_players(player)
